%% Code for generating null distribution for cross-stimulus generalization - this file runs on the cluster
% This script runs permutation testing for CPM models that predict across different stimuli
% (tool vs alcohol) while keeping phenotype and pill condition constant.
...
for pheno = pheno_ls
   for stim = stim_ls
       for pill = pill_ls
           % Define training and testing stimulus sets
           if stim == "alc"
               train_stim = "tool";
               test_stim = "alc";
               % Load training data (tool)
               filename = append(folder, 'n26_', pheno, '_', pill, '_tool_trialID_dynamic_connectome.mat');
           else
               train_stim = "alc";
               test_stim = "tool";
               % Load training data (alc)
           ...
           % Set output file path and prediction type based on phenotype
           if pheno == "RecogAcc_coded"
               predicted_type = "binary";
               outputfile = append('result/permutation/5s_onward_BPfiltered/', pheno, '_byTrial_', pill, '_', train_stim, '_', test_stim, '_p0.01.mat');
           else
               predicted_type = "linear";
               outputfile = append('result/permutation/5s_onward_BPfiltered/linear_', pheno, '_byTrial_', pill, '_', train_stim, '_', test_stim, '_p0.01.mat');
           end
