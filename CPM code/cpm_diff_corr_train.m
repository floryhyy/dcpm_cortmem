function [y_predict, performance, final_net, mean_net] = cpm_singlefunc(x,x_model,testX,y,y_model,testY,varargin)
dbstop if error
% (from cpm_main)
...
addParameter(p,'train_id',defaultid,@isnumeric);
addParameter(p,'test_id',defaultid,@isnumeric);
addParameter(p,'predicted_type',defaultid,@isstring);
...
id = p.Results.id;
test_id = p.Results.test_id;
train_id = p.Results.train_id;
predicted_type=p.Results.predicted_type;
...
[x,x_model,testX,y,no_nodes, symm_status]=cpm_check_errors(x,x_model,testX,y,kfolds,id);

%% Train & test Connectome-Based Predictive Model
[y_predict, final_net, mean_net]=cpm_cv_subj(x,x_model,testX,y,y_model,testY,id,train_id,test_id,pthresh,kfolds,no_nodes,predicted_type,flatten,symm_status);

%% Assess performance

[performance.mean.r_rank(1),performance.mean.r_rank(2)]=corr(y_predict.mean(:),testY(:), 'type', 'spearman');
[performance.mean.r_pearson(1),performance.mean.r_pearson(2)]=corr(y_predict.mean(:),testY(:), 'type', 'pearson');

[performance.neg.r_rank(1),performance.neg.r_rank(2)]=corr(y_predict.neg(:),testY(:), 'type', 'spearman');
[performance.neg.r_pearson(1),performance.neg.r_pearson(2)]=corr(y_predict.neg(:),testY(:), 'type', 'pearson');

[performance.pos.r_rank(1),performance.pos.r_rank(2)]=corr(y_predict.pos(:),testY(:), 'type', 'spearman');
[performance.pos.r_pearson(1),performance.pos.r_pearson(2)]=corr(y_predict.pos(:),testY(:), 'type', 'pearson');


performance.mean.mse = sum((y_predict.mean-testY).^2)/length(testY);
performance.neg.mse = sum((y_predict.neg-testY).^2)/length(testY);
performance.pos.mse = sum((y_predict.pos-testY).^2)/length(testY);
...
%% CHECK INPUTS
function [x,x_model,testX,y,no_nodes, symm_status] = cpm_check_errors(x,x_model,testX, y, folds, id)
...
if ndims(x)==3
    ...
    x=m;
    clear m;
    if issymmetric(x_model(:,:,1))
        symm_status = 1;
        s=size(x_model,1);
        for i=1:size(x_model,3)
            data_tmp=x_model(:,:,i);
            m(:,i)=data_tmp(logical(triu(ones(s,s),1)));
        end
    else
        symm_status = 0;
        m=reshape(x_model,size(x_model,1)*size(x_model,2),size(x_model,3));
    end

    x_model=m;
    clear m;

    if issymmetric(testX(:,:,1))
        symm_status = 1;
        s=size(testX,1);
        ...
        symm_status = 0;
        m=reshape(testX,size(testX,1)*size(testX,2),size(testX,3));
    end
    testX=m;
    clear m;
else 
...
%% CV
function [y_predict, final_net, mean_net]=cpm_cv_subj(x,x_model,testX,y,y_model,testY,id,train_id,test_id,pthresh,kfolds,no_nodes,predicted_type,flatten,symm_status)
...
% Split data
ndata=size(testX,2); %multiple observations per sub
...
for leftout = 1:kfolds
    ...
    x_m=x_model(:,ismember(train_id, traininds));
    y_m=y_model(ismember(train_id, traininds));
    x_test = testX(:,ismember(test_id, testinds));
...
%   y_train = (y_raw./mot(ismember(id, traininds)));
    ...
    [~, ~, pmask, mdl] = cpm_train(x_train,x_m,y_train,y_m,pthresh,predicted_type);
...
%%%% evg mod: store the edges that are predictive on every loop %%%%
    ...
    [y_predict_output] = cpm_test(x_test,mdl,pmask,predicted_type);
    y_predict.mean(ismember(test_id, testinds))= y_predict_output.mean;
    y_predict.pos(ismember(test_id, testinds))= y_predict_output.pos;
    y_predict.neg(ismember(test_id, testinds))= y_predict_output.neg;
...
%% TRAIN
function [r,p,pmask,mdl]=cpm_train(x,x_model,y,y_model,pthresh,predicted_type)
...
% For each subject, summarize selected features x
for i=1:size(x_model,2)
    summary_feature.mean(i)=nanmean(x_model(pmask>0,i))-nanmean(x_model(pmask<0,i));
    summary_feature.pos(i)=nanmean(x_model(pmask>0,i));
    summary_feature.neg(i)=-nanmean(x_model(pmask<0,i));
end
...
% Fit y to summary features
    if predicted_type=='categorical'
       [B,dev,stats]=mnrfit(summary_feature.mean,y_model,'model','ordinal');
        mdl.mean=B;
    elseif predicted_type=='binary'
        mdl.mean=glmfit(summary_feature.mean,y_model,'binomial','link','logit');
    else
        mdl.mean=robustfit(summary_feature.mean,y_model');
    end
...
if sum(isnan(summary_feature.pos))==length(summary_feature.pos)
 ...
 if predicted_type=="categorical"
       [B,dev,stats]=mnrfit(summary_feature.mean,y_model,'model','ordinal');
        mdl.pos=B;
    elseif predicted_type=="binary"
        mdl.pos=glmfit(summary_feature.mean,y_model,'binomial','link','logit');
    else
        mdl.pos=robustfit(summary_feature.mean,y_model');
 end
end
if sum(isnan(summary_feature.neg))==length(summary_feature.neg)
    ...
    if predicted_type=="categorical"
       [B,dev,stats]=mnrfit(summary_feature.mean,y_model,'model','ordinal');
        mdl.neg=B;
    elseif predicted_type=="binary"
        mdl.neg=glmfit(summary_feature.mean,y_model,'binomial','link','logit');
    else
        mdl.neg=robustfit(summary_feature.mean,y_model');
    end
end
...
%% TEST
function [y_predict]=cpm_test(x,mdl,pmask,predicted_type)
...
for i=1:size(x,2)
    ...
    elseif predicted_type=="binary"
        ...
        y_predict.mean(i)=mdl.mean(2)*summary_feature.mean(i) + mdl.mean(1);
        y_predict.pos(i)=mdl.pos(2)*summary_feature.pos(i) + mdl.pos(1);
        y_predict.neg(i)=mdl.neg(2)*summary_feature.neg(i) + mdl.neg(1);
    end
