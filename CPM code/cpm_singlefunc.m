...
[x,y,no_nodes, symm_status]=cpm_check_errors(x,y,kfolds,id,flatten);

%% Train & test Connectome-Based Predictive Model
[y_predict, final_net, mean_net]=cpm_cv_subj(x,y,id,pthresh,kfolds,no_nodes,flatten,symm_status);
...
%% CHECK INPUTS
function [x, y, no_nodes, symm_status] = cpm_check_errors(x, y, folds, id,flatten)
...
%% CV
function [y_predict, final_net, mean_net]=cpm_cv_subj(x,y,id,pthresh,kfolds,no_nodes,flatten,symm_status)
...
%%%% evg mod: store the edges that are predictive on every loop %%%%
    ...
    [y_predict_output] = cpm_test(x_test,mdl,pmask);
...
mdl.mean=robustfit(summary_feature.mean,y');
end

if sum(isnan(summary_feature.pos))==length(summary_feature.pos)
    mdl.pos=[0 0];
else
mdl.pos=robustfit(summary_feature.pos,y');
end
if sum(isnan(summary_feature.neg))==length(summary_feature.neg)
    mdl.neg=[0 0];
else
mdl.neg=robustfit(summary_feature.neg,y');
